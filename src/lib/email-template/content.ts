...
interface BuyerOrderStatusEmailParams {
...
interface BuyerDeliveredEmailParams {
  firstName: string;
  items: EmailTemplateItem[];
  trackingId: string;
}

interface BuyerCancelledEmailParams {
  firstName: string;
  items: EmailTemplateItem[];
  trackingId: string;
}
...
export const generateSimpleSellerOrderNotificationTemplate = (): string => `
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
export const generateBuyerOrderStatusEmail = ({ status, firstName, items, trackingId }: BuyerOrderStatusEmailParams): string => {
    ...
    const itemsHtml = items.map(item => `
        <div style="display: flex; gap: 15px; margin-bottom: 20px; border-bottom: 1px solid #eee; padding-bottom: 15px;">
            ${item.image ? `<img src="${item.image}" alt="${item.name}" width="80" style="border-radius: 4px;" />` : ''}
...
export const generateBuyerDeliveredEmail = ({ firstName, items, trackingId }: BuyerDeliveredEmailParams): string => {
    const itemsHtml = items.map(item => `
...
export const generateBuyerCancelledEmail = ({ firstName, items, trackingId }: BuyerCancelledEmailParams): string => {
    const itemsHtml = items.map(item => `
...
export const generateNegativeReviewAlertEmail = ({ customerName, productName, rating, review, orderId }: NegativeReviewAlertParams): string => {
    return `
    <!DOCTYPE html>
    <html lang="en">
    <head>
